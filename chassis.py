...
class TestChassis:
    ...
    __slots__ = "__wheels", "__position", "__angle", "__queue", "__init_encs"
    ...
    def __init__(self, starting_position, starting_angle):
        ...
        self.__wheels = {
            left = Wheel(__motors.left, 0.5 / math.pi, 8),
            right = Wheel(__motors.right, 0.5 / math.pi, 8)
        }
        self.__init_encs = { left = 0, right = 0 }
    ...
    def update(self):
        ...
        if self.__queue:
            if not self.__queue[0][0](self.__queue[0][1]):
                self.__queue.pop(0)
                self.__init_encs = {name: motor.get_encoder() for name, motor in self.__motors}
